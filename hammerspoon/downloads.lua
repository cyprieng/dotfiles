-- Function to delete files in Downloads folder older than 1 week
function cleanOldDownloads()
	...
	-- One week in seconds
	local oneWeek = 30 * 24 * 60 * 60

	-- Use AppleScript to get files from Downloads with their modification dates
	local script = [[
    tell application "Finder"
        set downloadsFolder to POSIX file "]] .. downloadsPath .. [[" as alias
        set downloadItems to items of downloadsFolder
        set itemList to {}
        repeat with i from 1 to count of downloadItems
            set thisItem to item i of downloadItems
				...
				if day and monthName and year and hour and min and sec then
					...
					if month then
						...
						if (currentTime - fileTime) > oneWeek then
							-- Use AppleScript to move the item to trash
							local moveScript = [[
                            tell application "Finder"
							...
							local moveSuccess = hs.osascript.applescript(moveScript)
							if moveSuccess then
								print("Moved old download to trash: " .. path)
							else
...
downloadsCleanupTimer = hs.timer.new(3600, cleanOldDownloads) -- Run every hour

-- Start the timer
downloadsCleanupTimer:start()
